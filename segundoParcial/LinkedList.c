...
int ll_count(LinkedList* this, int (pFunc)(void*,int),int cant)
{
    int index;
    LinkedList* ll_cumplen = ll_newLinkedList();
    void* auxElemento;
    int contador = 0;
    ...
    if(this != NULL && pFunc != NULL )
    {
            for(index=0;index<ll_len(this);index++)
            {
                ...
                if(pFunc(auxElemento,cant) == 1)
                {
                    contador++;
                    ll_add(ll_cumplen,auxElemento);
                }
            }
    }
    return contador;
